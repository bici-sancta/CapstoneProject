...
ped_crash_krnl <- read.csv(infile,
                       stringsAsFactors = FALSE, header = TRUE)
...
df <- merge(df, ped_crash_krnl, by = "cell_id", all.x = TRUE)
...
df$krnl_cost[is.na(df$krnl_cost)] <- 0
...
# ...
#> names(df)
# [1] "cell_id"           "ix"                "iy"                "lat.x"             "long.x"            "State"            
# [7] "County"            "City"              "neighborhood.x"    "RegionID"          "num_near_misses"   "lat_cell.x"       
#[13] "long_cell.x"       "access"            "dblprk"            "dnotyld"           "jywalk"            "lvisib"           
#[19] "lwfws"             "nobikef"           "noswlk"            "other"             "prkint"            "prkswlk"          
#[25] "speed"             "vrrlss"            "wlksig"            "xwalk"             "assist"            "bikes"            
#[31] "drives"            "other.1"           "walks"             "n_rqst"            "mean_walk_score"   "min_walk_score"   
#[37] "max_walk_score"    "num_walk_scores"   "sum_lane_cnt"      "sum_width"         "sum_area"          "num_streets"      
#[43] "lat.y"             "long.y"            "krnl_cost"         "num_events.x"      "num_fire_incd"     "med_sale_res_y"   
#[49] "med_sale_res_n"    "med_sale_com_y"    "med_sale_com_n"    "med_sale_ind_y"    "med_sale_ind_n"    "med_sale_pbo_y"   
#[55] "med_sale_pbo_n"    "lat_cell.y"        "long_cell.y"       "sum_cost"          "num_events.y"      "lat"              
#[61] "long"              "neighborhood.y"    "dist"              "stop_id"           "lat_stop"          "long_stop"        
#[67] "n_object"          "lat_cell"          "long_cell"         "animals_insects"   "building.related"  "construction"     
#[73] "food"              "others"            "police.property"   "service.complaint" "street_sidewalk"   "traffic_signal"   
#[79] "trash"             "trees_plants"      "water.leak"        "zoning_parking"    "n_request"
...
cols_2_drop <- c("neighborhood.x", "lat_cell.x", "long_cell.x", "requestid", "lat_cell", "long_cell", "lat", "long")
df_model[, cols_2_drop] <- NULL

df_model$krnl_cost <- NULL
df_model$num_events.x <- NULL

df_model <- cbind(df$krnl_cost, df$num_events.x, df_model)

names(df_model)[names(df_model) == "df$krnl_cost"] <- "krnl_cost_pedestrian_events"
names(df_model)[names(df_model) == "df$num_events.x"] <- "num_pedestrian_events"
names(df_model)[names(df_model) == "sum_cost"] <- "sum_cost_non_pedestrian_events"
names(df_model)[names(df_model) == "num_events.y"] <- "num_non_pedestrian_events"
...
hoods +
    geom_point(data = df_plot, aes(x = long.x, y = lat.x, color = log10(num_events.x)), shape = 19, size = 2.5, alpha = 0.9) + 
    geom_point(data = grid_centroid, aes(x = long, y = lat), color = "forestgreen", size = 0.2, alpha = 0.2) +
