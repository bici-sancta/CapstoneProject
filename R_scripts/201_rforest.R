...
tic("model 1.h2o")
model_rf1.h2o <- h2o.randomForest(y = y.dep,
                                  x = x.indep,
                                  training_frame = train.h2o)
toc()

#h2o.performance(model_rf1.h2o)
#import_list <- h2o.varimp(model_rf1.h2o)

# ...   predict ... test

train.h2o$pred <- h2o.predict(model_rf1.h2o, train.h2o)
test.h2o$pred <- h2o.predict(model_rf1.h2o, test.h2o)
...
#    table(df_test$error)
    ...
    cnf <- confusionMatrix(data = as.factor(qtwLx_hrf_test$pred),
                    reference = as.factor(qtwLx_hrf_test$target_y),
                    positive = "1")
    fp <- cnf$table[2]
...
plot(jitter(qtwLx_hrf_test$target_y) ~ jitter(qtwLx_hrf_test$hrf_prob),
     ...
     cex = 0.2,
     col = alpha("dodgerblue3", 0.4))



qtwLx_dnn_test$y_pred <- 0
qtwLx_dnn_test$y_pred[qtwLx_dnn_test$dnn_pred >= 0.5] <- 1
...
plot(jitter(qtwLx_hrf_test$target_y) ~ jitter(qtwLx_hrf_test$hrf_prob),
     ...
     sub = "(data values jittered for observability)",
     cex = 0.3,
     col = alpha((qtwLx_dnn_test$y_pred + 3), 0.4))
