...
# ...
# ...   organizer to prep data sets for submit to which_grid_cell_function()
# ...
...
df_mapped <- which_grid_cell_big(grid_centroid, near_miss)

# ...   -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
# ...   accumulate sum of costs in each grid cell
# ...   -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

near_miss_agg <- df_mapped %>% 
...
hoods +
    geom_point(data = near_miss_agg, aes(x = long_cell, y = lat_cell, color = num_near_misses), shape = 15, size = 2.5, alpha = 0.8) + 
